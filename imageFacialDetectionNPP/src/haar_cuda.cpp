...
void nearestNeighbor (ImageData *src, ImageData *dst);
...
std::vector<MyRect> detectObjects( ImageData* _img, MySize minSize, MySize maxSize, myCascade* cascade,
     ...
     **********************************/
    ImageData image1Obj;
    ImageDimensions sum1Obj;
    ImageDimensions sqsum1Obj;
    /* pointers for the created structs */
    ImageData *img1 = &image1Obj;
    ImageDimensions *sum1 = &sum1Obj;
    ImageDimensions *sqsum1 = &sqsum1Obj;
    ...
    if( minNeighbors != 0)
    {
        groupResults(allCandidates, minNeighbors, GROUP_EPS);
    }
...
void setImageForCascadeClassifier(myCascade* _cascade, ImageDimensions* _sum, ImageDimensions* _sqsum) {
    #ifdef FINAL_DEBUG
    printf("\n-- Entering setImageForCascadeClassifier --\n");
    #endif
    std::cout << "[DEBUG] Preparing image structures for cascade classification..." << std::endl;

    ImageDimensions *sum = _sum;
    ImageDimensions *sqsum = _sqsum;
    ...
    cascade->sum = *sum;
    cascade->sqsum = *sqsum;
    std::cout << "[DEBUG] Integral image structures assigned to cascade." << std::endl;
    ...
    std::cout << "[DEBUG] Filter rectangle dimensions: " << equRect.width << " x " << equRect.height << std::endl;
    ...
    {
        std::cerr << "[ERROR] Filter rectangle has at least one dimension equal to zero: " << equRect.width << " x " << equRect.height << std::endl;
    }

    int window_area = equRect.width * equRect.height;
    std::cout << "[DEBUG] Calculated window area: " << window_area << std::endl;
    ...
    std::cout << "[DEBUG] Calculated reverse window area: " << cascade->inv_window_area << std::endl;
    ...
    #endif
    std::cout << "[DEBUG] Setting integral image corner pointers in cascade... " << std::endl;
    ...
    #endif
    std::cout << "[DEBUG] Starting stage loop for cascade classification. Number of stages: " << cascade->n_stages << std::endl;

    for (i = 0; i < cascade->n_stages; i++) {
        //printf("  -- Stage: %d --\n", i);
        std::cout << "[DEBUG] Stage: " << i << ", number of features: " << cascade->stages_array[i] << std::endl;
        for (j = 0; j < cascade->stages_array[i]; j++) {
            //printf("    -- Feature: %d --\n", j);
            // std::cout << "[DEBUG] Stage " << i << ", feature " << j << ": " << std::endl;
            ...
            //std::cout << "[DEBUG] Finished processing feature, updating indices..." << std::endl;
            r_index += 12;  // 3 rectangles × 4 parameters each
            w_index += 3;   // 3 weights per feature
        }
        std::cout << "[DEBUG] Finished stage " << i << std::endl;
...
void integralImages( ImageData *src, ImageDimensions *sum, ImageDimensions *sqsum )
{
    std::cout << "[DEBUG] Calculating integral images..." << std::endl;
    int s, sq, t, tq;
    unsigned char it;
    int height = src->height;
    int width = src->width;
    std::cout << "[DEBUG] Source width: " << height << std::endl;
    std::cout << "[DEBUG] Source height: " << width << std::endl;
    unsigned char *data = src->data;
    int * sumData = sum->data;
    int * sqsumData = sqsum->data;
    std::cout << "[DEBUG] Source data: " << sumData << std::endl;
    std::cout << "[DEBUG] Square source data: " << sqsumData << std::endl;
    for( int y = 0; y < height; y++) {
        // std::cout << "[DEBUG] Column: " << y << std::endl;
        s = 0;
        sq = 0;
        /* loop over the number of columns */
        for( int x = 0; x < width; x++) {
            // std::cout << "[DEBUG] Column " << y << ", Row " << x << std::endl;
            it = data[y*width+x];
...
{
    std::cout << "Reading text classifier..." << std::endl;
    ...
    if (finfo == NULL) { // Check if file opened successfully
        std::cerr << "[ERROR] Unable to open info.txt." << std::endl;
    #ifdef FINAL_DEBUG
    ...
    #endif
    std::cout << "[DEBUG] Successfully opened info.txt." << std::endl;
    ...
    **************************************************/
    if (fgets(mystring, 12, finfo) != NULL)
    {
        ...
        #endif
        std::cerr << "[ERROR] Unable to read number of stages from info.txt." << std::endl;
        fclose(finfo);
        return;
    }
    i = 0;
    ...
    #endif
    std::cout << "[DEBUG] Reading stages array from info.txt." << std::endl;
    while (fgets(mystring, 12, finfo) != NULL)
    {
        stages_array[i] = atoi(mystring);
        std::cout << "[DEBUG] Stage " << i << " filters: " << stages_array[i] << std::endl;
        #ifdef FINAL_DEBUG
    ...
    #endif
    std::cout << "[DEBUG] Finished reading stages array from info.txt." << std::endl;
    ...
    std::cout << "[DEBUG] Finished allocating array structures." << std::endl;

    FILE* fp = fopen("class.txt", "rb");
    if (fp == NULL) { // Check if file opened successfully
        std::cerr << "[ERROR] Unable to open file class.txt." << std::endl;
    ...
    std::cout << "[DEBUG] Successfully opened file class.txt. Reading classifier parameters..." << std::endl;
    #ifdef FINAL_DEBUG
    ...
    {    /* loop over n of trees */
        ...
        {    /* loop over n of rectangular features */
            ...
            {    /* loop over the n of vertices */
                for (l = 0; l < 4; l++)
                {
                    if (fgets(mystring, 12, fp) != NULL) {
                        ...
                        std::cerr << "[ERROR] Unable to read rectangles_array at index " << r_index << " from class.txt." << std::endl;
                        #ifdef FINAL_DEBUG
                ...
                if (fgets(mystring, 12, fp) != NULL)
                {
                    ...
                    std::cerr << "[ERROR] Error reading weights_array at index " << w_index << " from class.txt." << std::endl;
                    #ifdef FINAL_DEBUG
            ...
            if (fgets(mystring, 12, fp) != NULL) {
                ...
                std::cerr << "[ERROR] Error reading tree_thresh_array at index " << tree_index << " from class.txt." << std::endl;
            ...
            if (fgets(mystring, 12, fp) != NULL) {
                ...
                std::cerr << "[ERROR] Error reading alpha1_array at index " << tree_index << " from class.txt." << std::endl;
            ...
            if (fgets(mystring, 12, fp) != NULL) {
                ...
                std::cerr << "[ERROR] Error reading alpha2_array at index " << tree_index << " from class.txt." << std::endl;
